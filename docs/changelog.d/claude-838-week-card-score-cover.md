- **#838** A `/week` scorecard now answers its own question. The header splits into identity
  (matchup + line) and result (score + verdict): the score leads its column in bold at the
  matchup's own size, instead of trailing the title row muted at body weight, with a status
  eyebrow above it carrying the live clock and the unofficial-final flag. A new cover readout
  names which side beat the number and by
