# Archived roadmap - 2026 offseason to Week 1

> Archived on 2026-06-22 when delivery tracking moved to GitHub Issues and
> architecture decisions moved to `docs/adr/`. This snapshot preserves the
> original feature detail for issue migration; it is not the source of truth for
> current status. See the [current roadmap](../../ROADMAP.md) and
...
### Drop your worst week — _near-term, ships independently_
...
### Line & lock system — _depends on Phase 3 cron (and Phase 4 push for alerts)_
...
- **Opt-in line-movement notification** (Phase 4 push): "your Chiefs line moved
  ...
  side/weight/All-In).
- A "Casual"/​big-jump-shift _grading_ mode was considered and cut — House already
...
### More catch-up — _after drop-worst-week_
...
- **Mulligan tokens** — N/season (2–3); **post-result, manual** void of a losing pick
  ...
  Highest LoE.
- Note: drop-worst-week and mulligans are both _forgiveness_ mechanics; only the boost
  helps you gain ground. Any bonus/multiplier widens the `pick_settlement` guard
  (`points_delta between -20 and 10`).

### Special weeks — _per-week rule overrides_
...
- **Lock of the Week** — flag one pick weekly for a small ±~3 modifier on top of its
  weight (kept a _modifier_, not another 10-pointer, to stay distinct from All-In).
