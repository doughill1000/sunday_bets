...
- **fix** Svelte `state_referenced_locally` warnings — wrapped seed-value `$state()` initializers with `untrack()` in `WelcomeGuide` and `CommentsSection` so prop references captured once at mount are explicitly marked non-reactive. files: src/lib/components/howto/WelcomeGuide.svelte, src/lib/components/picks/CommentsSection.svelte
