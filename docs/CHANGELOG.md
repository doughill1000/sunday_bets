...
- **Mobile header layout** — group switcher now sits in the top-left on mobile (desktop unchanged: it stays top-right next to the avatar). Fills the empty left slot left by the hidden desktop nav and stops the multi-group chip from overlapping the centered logo. A single `GroupSwitcher` repositioned by breakpoint via auto-margins. component: `AppHeader`
