...
**Hotshot** — a private NFL pick'em PWA for ~6 friends. Players pick a team against
...
(The repo and directory are still named `sunday_bets`; ADR-0027's rename is
product-name only.)
...
- **UI has a merge gate (ADR-0030 / ADR-0029):** `docs/DESIGN.md` governs every
  ...
  radiogroup to switch a cut, never a second nested control. Token vocabulary lives in
  `docs/agent-context/design-system.md`; the raw-hex guard rejects inline colours.
...
> **Claude Code sessions:** skills automate most of the stages below (`issue-author`
> → `start-issue` → `finish-pr` → `land-pr` → `cut-release`, plus utilities) — see the
> skills map in `CLAUDE.md`. Codex and other agents follow the same steps manually.
...
In-app feedback (ADR-0028) is a deliberate exception to the group-tenancy boundary
(ADR-0002): `feedback` rows are **user-scoped**, not group-scoped, and reach GitHub
only through an admin-gated, sanitized server egress — read the ADR before building
on it.

## Operations & observability

- **Cron health:** `/api/health` (token-guarded, read-only) flags missed-run and
  stale-data status across the prod crons plus the odds-API cap; an external monitor
  polls it (this replaced paid Sentry cron monitors). Pausing crons for the offseason
  means pausing that monitor too. See `docs/observability/health-watchdog.md`
  (thresholds in `docs/observability/scaling-signals.md`); run the `season-ops` skill
  for a one-shot readiness / week-health check.
- **Runbooks:** operational procedures not covered by CI live in `docs/runbooks/`
  (prod/staging reconciliation, iOS-PWA install + push verification).
- **Audits:** periodic repo pattern and behavior audits land in `docs/audits/`,
  generated by the `pattern-audit` skill.
