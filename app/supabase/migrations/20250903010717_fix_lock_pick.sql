...
begin
  ...
  -- FIX: compute week_number/season_id, then compare to season max
  select w.week_number, w.season_id
  into v_week_number, v_season_id
  from public.weeks w
  where w.id = v_week_id;

  select max(w.week_number)
  into v_last_week_number
  from public.weeks w
  where w.season_id = v_season_id;

  v_is_last_week := v_last_week_number = v_week_number;
...
grant execute on function public.lock_pick(uuid, text, weight_enum) to authenticated, service_role;