...
-- ENUMS
do $$ begin
  create type weight_enum as enum ('L','M','H','A');
...
create table if not exists public.users (
  ...
  role text not null default 'player', -- 'player' | 'admin' (mirrors JWT claim)
...
create table if not exists public.games (
  ...
  espn_event_id text unique,             -- filled after first ESPN match
...
create table if not exists public.picks (
  ...
  relock_used boolean not null default false,
  -- line snapshots for grading (final = last lock)
