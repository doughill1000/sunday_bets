...
-- One directional row per player pair per (group, season), counting only games where
-- the two players backed opposite teams (different picked_team_id). Games where both
-- backed the same team are excluded — those measure confidence, not disagreement. Missed
-- picks (null pick_id in pick_settlement) are inner-dropped and therefore never counted.
