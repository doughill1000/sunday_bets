...
-- Per-pick against-the-spread cover margin and declared weight for the weekly-hardware
-- awards (issue #387, extended #866). One row per settled real pick (pick_id not null) in
-- a scoring round, at the (group_id, game_id, user_id) grain -- the same grain as
...
-- alongside it.
--
-- weight (#866): picks.weight as declared at lock, feeding the Bullseye award (an All-In that
-- won). Same value grading scored on, so the award cannot disagree with the points already
-- paid; it reads the settled result and re-scores nothing (ADR-0023 unchanged).
--
-- #866 retired the cover-margin awards (Bad Beat / Backdoor); cover_margin stays as the
-- view's namesake and canonical per-settled-pick ATS margin, covered by supabase/tests/052.
--
