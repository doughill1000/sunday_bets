...
-- 1) Uniqueness on (user_id, game_id)
create unique index if not exists uq_picks_user_game
  on public.picks (user_id, game_id);
...
begin
  if not exists (
    ...
    where conrelid = 'public.picks'::regclass
      and conname = 'uq_picks_user_game'
  ) then
    execute 'alter table public.picks add constraint "uq_picks_user_game" unique using index "uq_picks_user_game"';
  end if;
end$$;
...
drop function if exists public.ats_margin_at_lock(int, int, uuid, uuid, uuid, numeric);
drop view if exists public.current_active_line;
drop view if exists public.game_lines_normalized;
