-- pgTAP test bootstrap (offline).
--
-- Installs pgTAP and the basejump "supabase_test_helpers" used by the pgTAP suite
-- (tests.create_supabase_user / get_supabase_user / get_supabase_uid /
-- authenticate_as / clear_authentication, plus rls_enabled / freeze_time helpers).
-- The helper DDL below is VENDORED verbatim from upstream so this bootstrap has no
-- network dependency:
--   source  : https://github.com/usebasejump/supabase-test-helpers
--   file    : supabase_test_helpers--0.0.6.sql
--   vendored: 2026-06-26
-- The ONLY change from upstream is the removal of the psql `\echo ... \quit`
-- CREATE EXTENSION load-guard (its first two lines) so the file can be sourced
-- directly here. To re-vendor: re-download that file, strip the two guard lines,
-- and replace the block between the BEGIN/END markers below.
--
-- Previously this fetched the dbdev package manager over HTTP from api.database.dev
-- (pg_tle + pgsql-http) on every run, so the entire pgTAP suite failed whenever that
-- endpoint was slow or unreachable. This is the ADR-0011 "pgTAP offline-bootstrap
-- hardening" follow-up (see ADR-0012).

-- install pgtap extension for testing
create extension if not exists pgtap with schema extensions;

-- ============================================================================
-- BEGIN vendored supabase_test_helpers v0.0.6
-- ============================================================================
...
-- ============================================================================
-- END vendored supabase_test_helpers v0.0.6
-- ============================================================================
