...
-- pgTAP for ADR-0005 drop-worst-week in leaderboard_season_totals.
-- Covers: clear-minimum drop, uniform-score drop, <2-week no-op, points-only
-- record handling, and cross-group config isolation (drop on vs drop off).
...
-- Players -------------------------------------------------------------------
select tests.create_supabase_user('dw_clear');   -- 3 distinct weekly scores
...
-- Group A enables drop-worst-week; Group B has no config row (defaults off).
...
insert into dw_data values
  ...
  -- Group B, same scores, drop disabled
