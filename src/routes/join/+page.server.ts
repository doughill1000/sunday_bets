...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const canCreate = await canCreateGroup(locals.user.id);
  return { canCreate };
};
...
export const actions: Actions = {
  create: async ({ request, locals }) => {
    ...
    // SECURITY DEFINER gate. The RPC enforces the gate authoritatively. There is no
    // start-week argument: a new league always begins at now() (ADR-0039), which is the
    // column default, and joining is the only participation boundary.
