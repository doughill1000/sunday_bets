...
describe('upsertGameByExternalId command', () => {
  it('throws on rpc error', async () => {
    ...
    await expect(upsertGameByExternalId({ externalGameId: 'x', weekId: 1, commenceTime: '2024-01-01T00:00:00Z', homeTeamId: 1, awayTeamId: 2 })).rejects.toThrow('fail');
  });
  it('throws when rpc returns no id', async () => {
    mockRpc.mockResolvedValueOnce({ data: null, error: null });
    await expect(upsertGameByExternalId({ externalGameId: 'x', weekId: 1, commenceTime: '2024-01-01T00:00:00Z', homeTeamId: 1, awayTeamId: 2 })).rejects.toThrow(/returned no id/);
  });
  it('returns id when successful', async () => {
    mockRpc.mockResolvedValueOnce({ data: 'uuid-123', error: null });
    const id = await upsertGameByExternalId({ externalGameId: 'x', weekId: 1, commenceTime: '2024-01-01T00:00:00Z', homeTeamId: 1, awayTeamId: 2 });
