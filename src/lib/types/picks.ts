...
export type PickEntry = {
  ...
  /**
   * The settled `pick_settlement.outcome` for this game once grading has run (#823), or
   * `undefined`/`null` while it is still ungraded. Grading is the sole settlement authority
   * (ADR-0007) — this is a read of what it wrote, never a client-side derivation, and it
   * always outranks the display-only live projection (see `$lib/domain/pickResult`).
