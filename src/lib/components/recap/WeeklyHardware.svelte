<script lang="ts">
  ...
  let {
    ...
    /** When set, a link out to the week's story renders at the top of the card (#631, moved
     *  above the tiles in #866). The League Week tab passes it so the recap is one tap away
     *  without inlining a RecapCard; the Season recaps archive leaves it null because the card
     *  already sits right below. */
  ...
  function detailText(a: WeeklyAward): string {
    switch (a.id) {
      ...
      // No ranked stat: co-winners hold different picks, so the count is the honest line.
      case 'bullseye':
        return a.holders.length > 1 ? `${a.holders.length} All-Ins landed` : 'All-In landed';
    }
  }
</script>
...
<Card class="border-border/50 bg-card">
  ...
  <CardContent class="space-y-3">
    <!-- Above the tiles since #866: below them, the card's one navigational element sat
         behind a block that grows with the catalog. -->
    {#if recapHref}
      <a
        href={recapHref}
