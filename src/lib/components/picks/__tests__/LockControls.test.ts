...
describe('LockControls (Lock in button)', () => {
  ...
  it('shows the in-flight label while a lock-in is saving', () => {
    ...
    render(LockControls, { props: { game, started: false } });
    expect(screen.getByTestId('lock-in')).toHaveTextContent('Locking in…');
    expect(screen.getByTestId('lock-in')).toBeDisabled();
  });

  it('surfaces an error and re-saves when Lock in is tapped again', async () => {
    setPicks({
    ...
    render(LockControls, { props: { game, started: false } });
    expect(screen.getByText(/Couldn.t lock in/)).toBeInTheDocument();
  ...
  it('Clear pick removes a staged pick without calling unlock', async () => {
    setPicks({ g1: { selected: { team: 'home', weight: 'M' } } });
