...
describe('isWithinLiveWindow', () => {
  ...
  // The window is sized by the grade cron, not by game length (#823). The binding case is the
  // late-season Saturday slate: `cron-grade.yml` grades it only at Sun 04:00 UTC, so a Saturday
  // 1pm ET final has to stay covered for ~11h or it falls into a gap where the feed has aged out
  // and nothing has been settled — which is what left /picks showing a bare "⏱ Kicked off".
