 ...
 *
 * SIZED BY THE GRADE CRON, not by game length (#823). The window has to reach the grade that
 * replaces it, or a finished game falls into a gap where the feed has aged out and nothing has
 * been settled yet — which is what left `/picks` showing a bare "⏱ Kicked off". At 6h the gap
 * was routine; the binding case is the late-season **Saturday** slate, which `cron-grade.yml`
 * grades only at Sun 04:00 UTC — a Saturday 1pm ET final (≈17:00 UTC) needs ~11h of cover. The
 * Sunday slate (graded hourly, Sun 21:00–Mon 06:00 UTC) and TNF/MNF already land at or inside
 * their close. 12h covers the Saturday case with a little room and nothing wider.
 *
 * The cost of a wider window is bounded on the other end rather than by this number: the client
 * stops repolling once every in-window game reports `final` (`PicksBoard`), and a graded game is
 * excluded from the window outright, so a load in the gap costs ONE ESPN fetch to learn the game
 * is over and then holds. The shared server cache still collapses concurrent viewers to ≤1 fetch
 * regardless of headcount, so this does not scale with the audience.
 */
